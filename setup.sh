...
restore_and_exit() {
    local step_name="$1"
    echo -e "\n\033[1;31m❌ Ocurrió un error crítico durante el paso: '$step_name'.\033[0m"
...
check_deps() {
    ...
    for cmd in docker curl wget sed grep cut xargs; do if ! command -v $cmd &> /dev/null; then echo "❌ Error: El comando '$cmd' es esencial." && exit 1; fi; done
    ...
    if [ ! -f ./yq ]; then
        ...
        YQ_VERSION="v4.30.8"; YQ_BINARY="yq_linux_amd64"; wget -q "https://github.com/mikefarah/yq/releases/download/${YQ_VERSION}/${YQ_BINARY}" -O ./yq && chmod +x ./yq || { echo "❌ Falló la descarga de yq." && exit 1; }
    fi
...
if [ ! -f "$N8N_COMPOSE_PATH" ]; then echo "❌ Error: No se encontró 'docker-compose.yml' en el directorio actual." >&2; rm -f yq; exit 1; fi
...
if [ -z "$DETECTED_NETWORK" ] || [ "$DETECTED_NETWORK" == "null" ]; then echo "❌ Error: No se pudo detectar la red." >&2 && restore_and_exit "Detección de Red"; fi
...
# Comprobar si el modo 'queue' ya está activado en su forma moderna
...
if [ -z "$IS_QUEUE_MODE" ] || [ "$WORKER_EXISTS" == "false" ]; then
    echo "🔧 La configuración de escalado no está activa o completa. Se procederá a (re)configurar 'docker-compose.yml'."
    ...
    echo "⚙️  Aplicando configuración moderna de escalado (paso a paso)..."

    # --- MODIFICACIONES MODERNAS ---
    echo "   Paso 1/4: Eliminando variables obsoletas (EXECUTIONS_PROCESS)..."
    $YQ_CMD eval -i 'del(.services.[].environment[] | select(. == "EXECUTIONS_PROCESS=main" or . == "EXECUTIONS_PROCESS=worker"))' "$N8N_COMPOSE_PATH" || restore_and_exit "Paso 1"

    echo "   Paso 2/4: Añadiendo variables modernas al servicio principal..."
    # Limpiamos el host de redis para evitar espacios
    CLEAN_REDIS_HOST=$(echo "$REDIS_HOST" | xargs)
    $YQ_CMD eval -i ".services.\"$N8N_MAIN_SERVICE_NAME\".environment += [\"EXECUTIONS_MODE=queue\", \"QUEUE_BULL_REDIS_HOST=$CLEAN_REDIS_HOST\", \"OFFLOAD_MANUAL_EXECUTIONS_TO_WORKERS=true\"]" "$N8N_COMPOSE_PATH" || restore_and_exit "Paso 2"

    echo "   Paso 3/4: Creando/Actualizando el servicio worker..."
    $YQ_CMD eval -i ".services.\"$N8N_WORKER_SERVICE_NAME\" = .services.\"$N8N_MAIN_SERVICE_NAME\"" "$N8N_COMPOSE_PATH" || restore_and_exit "Paso 3"

    echo "   Paso 4/4: Limpiando claves innecesarias del worker..."
    $YQ_CMD eval -i "del(.services.\"$N8N_WORKER_SERVICE_NAME\".ports) | del(.services.\"$N8N_WORKER_SERVICE_NAME\".container_name) | del(.services.\"$N8N_WORKER_SERVICE_NAME\".labels)" "$N8N_COMPOSE_PATH" || restore_and_exit "Paso 4"

    echo "✅ Modificación de 'docker-compose.yml' completada con éxito."
    ...
    $COMPOSE_CMD_HOST -p "$N8N_PROJECT_NAME" up -d --force-recreate --remove-orphans || restore_and_exit "Reinicio de Docker"
    echo "✅ Tu stack de n8n ha sido actualizado y reiniciado con éxito."
else
    echo "✅ La configuración de escalado ya está activa. No se realizarán cambios."
fi
...
if [ -f "$N8N_ENV_PATH" ]; then cp "$N8N_ENV_PATH" "$AUTOSCALER_DIR/.env"; else touch "$AUTOSCALER_DIR/.env"; fi
cd "$AUTOSCALER_DIR" || exit

echo -e "\nAhora, configuremos el comportamiento del auto-escalado (con valores por defecto mejorados):"
QUEUE_THRESHOLD=$(ask "Nº de tareas en cola para crear un nuevo worker" "15")
MAX_WORKERS=$(ask "Nº máximo de workers permitidos" "5")
MIN_WORKERS=$(ask "Nº mínimo de workers activos (Recomendado: 1 para tener siempre uno disponible)" "1")
...
# Crear/Añadir las variables del autoscaler al .env
...
def send_telegram_notification(message):
    ...
    except requests.exceptions.RequestException as e: log(f"⚠️  Error al enviar notificación a Telegram: {e}")
def run_docker_command(command):
    try:
        ...
        error_message = f"❌ Error ejecutando Docker: {e.stderr.strip()}"; log(error_message); send_telegram_notification(f"‼️ *Error Crítico de Docker*\n_{e.stderr.strip()}_"); return None
...
def scale_workers(desired_count):
    ...
    if run_docker_command(command) is not None: log(f"✅ Escalado completado. Workers activos: {desired_count}"); send_telegram_notification(f"✅ Auto-escalado de *{N8N_PROJECT_NAME}*. Workers activos: *{desired_count}*")
    else: log(f"❌ Error al intentar escalar a {desired_count} workers."); send_telegram_notification(f"❌ *Error al escalar workers a {desired_count}*")
def main_loop():
    idle_since = None
    while True:
        try:
            ...
            if queue_size > (running_workers * QUEUE_THRESHOLD) and running_workers < MAX_WORKERS:
                scale_workers(min(running_workers + 1, MAX_WORKERS)); idle_since = None
            elif queue_size == 0 and running_workers > MIN_WORKERS:
                if idle_since is None: idle_since = time.time(); log(f"La cola está vacía. Iniciando temporizador de {IDLE_TIME_BEFORE_SCALE_DOWN}s para scale-down.")
                if time.time() - idle_since >= IDLE_TIME_BEFORE_SCALE_DOWN: scale_workers(max(running_workers - 1, MIN_WORKERS)); idle_since = None
            elif queue_size > 0:
                if idle_since is not None: log("La cola ya no está vacía. Cancelando scale-down."); idle_since = None
            time.sleep(POLL_INTERVAL)
        except redis.exceptions.RedisError as e: log(f"⚠️ Error de conexión con Redis: {e}. Reintentando..."); time.sleep(POLL_INTERVAL * 2)
        except KeyboardInterrupt: log("🛑 Script detenido por el usuario."); send_telegram_notification(f"🤖 Servicio para *{N8N_PROJECT_NAME}* detenido."); break
...
if __name__ == "__main__":
    ...
    QUEUE_KEY = "bull:n8n-executions:wait"; QUEUE_THRESHOLD = int(os.getenv('QUEUE_THRESHOLD', 15)); MAX_WORKERS = int(os.getenv('MAX_WORKERS', 5))
    ...
    try:
        redis_client = redis.Redis(host=REDIS_HOST, port=6379, db=0, decode_responses=True, socket_connect_timeout=5); redis_client.ping(); log("✅ Conexión con Redis establecida con éxito.")
    except redis.exceptions.RedisError as e: log(f"❌ Error fatal al conectar con Redis en {REDIS_HOST}: {e}"); exit(1)
    log(f"🚀 Iniciando servicio de auto-escalado para el proyecto '{N8N_PROJECT_NAME}'"); send_telegram_notification(f"🤖 El servicio de auto-escalado para *{N8N_PROJECT_NAME}* ha sido (re)iniciado.")
    main_loop()
EOL
...
if [ $? -eq 0 ]; then
    ...
    cd ..; echo "Tu stack de n8n ha sido configurado para escalar y el servicio de auto-escalado está en funcionamiento."; echo ""
    echo "Pasos siguientes recomendados:"; echo "  1. Revisa los logs: \033[0;32mdocker logs -f ${N8N_PROJECT_NAME}_autoscaler\033[0m"
    echo "  2. Configuración en: \033[0;32m./n8n-autoscaler/\033[0m"
else
