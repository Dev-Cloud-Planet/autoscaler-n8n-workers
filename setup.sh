...
check_deps() {
    echo "🔎 Verificando dependencias..."; for cmd in docker curl wget sed grep cut xargs; do if ! command -v $cmd &> /dev/null; then echo "❌ Error: El comando '$cmd' es esencial." && exit 1; fi; done
...
# --- INICIO DEL SCRIPT ---
clear
print_header "Instalador del Servicio de Auto-Escalado para n8n v2.5"
check_deps
...
if [ -f "$(pwd)/.env" ]; then echo "✅ Archivo de entorno '.env' detectado."; N8N_ENV_PATH="$(pwd)/.env"; DEFAULT_REDIS_HOST=$(grep -E "^REDIS_HOST=" "$N8N_ENV_PATH" | cut -d '=' -f2 | tr -d '"\''); DEFAULT_PROJECT_NAME=$(grep -E "^COMPOSE_PROJECT_NAME=" "$N8N_ENV_PATH" | cut -d '=' -f2 | tr -d '"\''); fi
...
DETECTED_NETWORK=$($YQ_CMD eval ".services.\"$N8N_MAIN_SERVICE_NAME\".networks[0]" "$N8N_COMPOSE_PATH"); if [ -z "$DETECTED_NETWORK" ] || [ "$DETECTED_NETWORK" == "null" ]; then echo "❌ Error: No se pudo detectar la red del servicio '$N8N_MAIN_SERVICE_NAME'." && restore_and_exit; fi
...
if [ -z "$IS_QUEUE_MODE" ]; then
    ...
    BACKUP_FILE="${N8N_COMPOSE_PATH}.backup.$(date +%F_%H-%M-%S)"; echo "🛡️  Creando copia de seguridad en '$BACKUP_FILE'..."; cp "$N8N_COMPOSE_PATH" "$BACKUP_FILE"
    echo "⚙️  Aplicando configuración de modo 'queue' y añadiendo servicio de worker..."
    
    ### CORRECCIÓN FINALÍSIMA: El comando yq sin ningún comentario dentro ###
    $YQ_CMD eval "
    ...
    echo "🔄 Deteniendo y levantando los servicios con la nueva configuración..."; $COMPOSE_CMD_HOST -p "$N8N_PROJECT_NAME" up -d --force-recreate --remove-orphans || restore_and_exit
...
def docker_cmd(cmd):
    try:
        ...
        err=f"❌ Docker Error: {e.stderr.strip()}";log(err);notify(f"‼️ *Docker Error*\n_{e.stderr.strip()}_");return None
def get_workers():
