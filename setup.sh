...
# ==============================================================================
#      Script de Instalación Avanzado del Servicio de Auto-Escalado para n8n
#
# Este script:
# 1. Detecta si el stack de n8n está preparado para el modo 'queue'.
# 2. Si no lo está, modifica el 'docker-compose.yml' existente para añadirlo:
#    - Hace una copia de seguridad del archivo original.
#    - Añade las variables de entorno para Redis y el modo 'queue'.
#    - Clona el servicio principal de n8n para crear un servicio 'n8n-worker'.
# 3. Despliega un servicio de 'autoscaler' para gestionar los workers.
# ==============================================================================
...
ask() {
    ...
    prompt="$1"
    default="$2"
    read -p "$prompt (def: $default): " reply
    echo "${reply:-$default}"
}
...
check_deps() {
    ...
    # Verificar e instalar yq si no existe
    if ! command -v yq &> /dev/null; then
        echo "⚠️ 'yq' no encontrado. Es necesario para modificar archivos YAML de forma segura."
        read -p "¿Deseas instalar 'yq' (v4) ahora? (y/N): " confirm_yq
        if [[ "$confirm_yq" =~ ^[yY](es)?$ ]]; then
            echo "📥 Instalando yq..."
            sudo wget https://github.com/mikefarah/yq/releases/latest/download/yq_linux_amd64 -O /usr/bin/yq && sudo chmod +x /usr/bin/yq
            if ! command -v yq &> /dev/null; then
                echo "❌ Falló la instalación de yq. Por favor, instálalo manualmente e inténtalo de nuevo."
                exit 1
            fi
            echo "✅ 'yq' instalado correctamente."
        else
            echo "Instalación cancelada. 'yq' es requerido para continuar."
            exit 1
        fi
    fi
...
echo "Este script configurará tu stack de n8n para escalado automático."
echo "Modificará tu 'docker-compose.yml' existente si es necesario."

# --- RECOPILACIÓN DE DATOS ---
print_header "1. Ubicación de tu Stack de n8n"
N8N_COMPOSE_PATH=$(ask "Introduce la ruta completa a tu archivo 'docker-compose.yml' de n8n" "$(pwd)/docker-compose.yml")
if [ ! -f "$N8N_COMPOSE_PATH" ]; then
    echo "❌ Error: El archivo '$N8N_COMPOSE_PATH' no existe."
    exit 1
fi
# Extraer el directorio y el nombre del archivo
N8N_COMPOSE_DIR=$(dirname "$N8N_COMPOSE_PATH")
N8N_PROJECT_NAME=$(basename "$N8N_COMPOSE_DIR")

print_header "2. Configuración de Conexión a Redis"
REDIS_HOST=$(ask "Introduce el Host o IP de tu servidor Redis" "redis")
REDIS_PORT=$(ask "Introduce el Puerto de tu servidor Redis" "6379")
REDIS_PASSWORD=$(ask "Introduce la Contraseña de Redis (opcional)" "")

print_header "3. Nombres de los Servicios"
N8N_MAIN_SERVICE_NAME=$(ask "Introduce el nombre de tu servicio principal de n8n en el YAML" "n8n")
N8N_WORKER_SERVICE_NAME="n8n-worker" # Forzamos un nombre estándar para el nuevo worker

# --- MODIFICACIÓN DEL DOCKER-COMPOSE DE N8N ---
cd "$N8N_COMPOSE_DIR" || exit

print_header "4. Preparando el Stack de n8n para Escalado"
echo "Analizando '$N8N_COMPOSE_PATH'..."

# Verificar si el servicio de worker ya existe
if yq e ".services | has(\"$N8N_WORKER_SERVICE_NAME\")" "$N8N_COMPOSE_PATH" | grep -q "true"; then
    ...
    read -p "¿Estás de acuerdo en modificar tu 'docker-compose.yml'? (Se creará una copia de seguridad) (y/N): " confirm_modify
    ...
    # Añadir/actualizar variables de entorno para el modo 'queue'
    ...
    # Modificar el servicio worker clonado
    yq e -i ".services.$N8N_WORKER_SERVICE_NAME.environment.EXECUTIONS_PROCESS = \"worker\"" "$N8N_COMPOSE_PATH"
    yq e -i "del(.services.$N8N_WORKER_SERVICE_NAME.ports)" "$N8N_COMPOSE_PATH" # Los workers no necesitan puertos expuestos
    ...
    docker-compose -p "$N8N_PROJECT_NAME" up -d --remove-orphans
...
AUTOSCALER_PROJECT_DIR="${N8N_COMPOSE_DIR}/n8n-autoscaler"
...
echo "Creando directorio del proyecto en '${AUTOSCALER_PROJECT_DIR}'..."
...
def scale_workers(desired_count):
    ...
    if run_docker_command(command) is not None:
        message = f"✅ Worker auto-escalado. *Workers activos: {desired_count}*"
        print("✔️ Comando de escalado ejecutado.")
