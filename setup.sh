...
#
#   Versión: 3.0 (Estructura estable, lógica de yq corregida y sin minificar)
# ==============================================================================
...
if [ -z "$IS_QUEUE_MODE" ]; then
    ...
    # Mensaje simplificado para evitar errores de sintaxis
    ...
    ### COMANDO YQ LIMPIO Y SIN COMENTARIOS ###
    $YQ_CMD eval "
        .services.\"$N8N_MAIN_SERVICE_NAME\".environment += [\"EXECUTIONS_MODE=queue\",\"EXECUTIONS_PROCESS=main\",\"QUEUE_BULL_REDIS_HOST=$REDIS_HOST\"] |
...
def send_telegram_notification(message):
    ...
    try: requests.post(f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage", json={'chat_id': TELEGRAM_CHAT_ID, 'text': message, 'parse_mode': 'Markdown'}, timeout=10).raise_for_status()
    except: log(f"⚠️  Error al enviar notificación a Telegram")
...
def main_loop():
    ...
    while True:
        try:
            ...
            if queue_size > QUEUE_THRESHOLD and running_workers < MAX_WORKERS: scale_workers(min(running_workers + 1, MAX_WORKERS)); idle_since = None
...
if __name__ == "__main__":
    load_dotenv(); REDIS_HOST = os.getenv('REDIS_HOST', 'redis'); N8N_PROJECT_NAME = os.getenv('N8N_DOCKER_PROJECT_NAME'); N8N_WORKER_SERVICE_NAME = os.getenv('N8N_WORKER_SERVICE_NAME'); QUEUE_KEY = "bull:n8n-executions:wait"
    QUEUE_THRESHOLD = int(os.getenv('QUEUE_THRESHOLD', 15)); MAX_WORKERS = int(os.getenv('MAX_WORKERS', 5)); MIN_WORKERS = int(os.getenv('MIN_WORKERS', 0))
    IDLE_TIME_BEFORE_SCALE_DOWN = int(os.getenv('IDLE_TIME_BEFORE_SCALE_DOWN', 90)); POLL_INTERVAL = int(os.getenv('POLL_INTERVAL', 10)); TELEGRAM_TOKEN = os.getenv('TELEGRAM_BOT_TOKEN'); TELEGRAM_CHAT_ID = os.getenv('TELEGRAM_CHAT_ID')
    if not all([N8N_PROJECT_NAME, N8N_WORKER_SERVICE_NAME]): log("❌ Error: Faltan variables de entorno críticas."); exit(1)
    try: redis_client = redis.Redis(host=REDIS_HOST, port=6379, db=0, decode_responses=True, socket_connect_timeout=5); redis_client.ping(); log("✅ Conexión con Redis establecida con éxito.")
